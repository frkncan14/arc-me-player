 ...
 *
 */
public final class CommonSet implements ICommonSet {
  ...
  //private final static boolean isM = true;
  ...
  private CommonSet(Context context) {
    mAudManager = (AudioManager) context.getSystemService(Context.AUDIO_SERVICE);
  ...
  public static synchronized CommonSet getInstance(Context context) {
    if (mmpCom == null) {
  ...
  public void mmpRestoreVideoResource() {
    ...
    if (Util.getPvrPlaying()) {
      Log.d(TAG, "mmpRestoreVideoResource do nothing");
      // do nothing;
    } else if (blackmute != -1) {
  ...
  public void setDisplayRegionToFullScreen() {
    Log.d(TAG, "setDisplayRegionToFullScreen = ",new Throwable());
