...
public class WifiAvailableAp {
	...
	private final static int PING_SUPPLICANT_SUCCESS = 1;
	private final static int PING_SUPPLICANT_FAILED = 2;
	 ...
	 *
	 */
	public static synchronized WifiAvailableAp getInstance(Context context) {
...
//			Log.d(TAG, "number[" + i + "] "+ list.get(i).SSID + " " + list.get(i).toString());
	...
	public boolean enableNetworkLink(ScanResult result) {
		...
		if (isConfigured(result)) {
			if (localLOGV) {
                NetLog.d(TAG, "[WifiAvailableAp][EnableNetworkLink]: " + result.SSID + " has been configured.");
            }
			networkId = getNetworkId(result);
		} else {
			if (localLOGV) {
                NetLog.d(TAG, "[WifiAvailableAp][EnableNetworkLink]: " + result.SSID + " has not been configured.");
            }
