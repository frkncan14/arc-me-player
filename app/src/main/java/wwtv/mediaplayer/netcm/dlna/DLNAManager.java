...
public class DLNAManager implements DeviceEventListener, ContentEventListener {
  ...
  private final static int CONTENT_PARENT = 0;
  private final static int CONTENT_CHILD = 1;
  private final static int CONTENT_UNKNOW = 2;
  private static DLNAManager mDLNAManager = null;
  private MediaServer server = null;
  private DigitalMediaPlayer player;
  private final static int BROWSE_REQUEST = 30;
  private int REQUEST_MATCH_NUM = 0;
  private int mTotalNum = 0;
  private String currentID;
  private static List<DLNAFile> mListDLNADeviceName = new LinkedList<DLNAFile>();
  private static Map<String, DLNADevice> mNameDeviceTable
  = new Hashtable<String, DLNADevice>();
  private static List<DLNAFile> mListContentName = new LinkedList<DLNAFile>();
  private static Map<String, Content> mNameContentTable = new Hashtable<String, Content>();
  private static Map<String, Content> mSubTitleContentTable
  = new Hashtable<String, Content>();
  private final Stack<DLNATempFile> mTraceFile = new Stack<DLNATempFile>();
  private final static String mRoot = "/";
  ...
  class DLNATempFile {
    ...
    public boolean equalsInfo(DLNATempFile fileIns) {
      int type = this.getType();
  ...
  private boolean isDevice(String fileName) {
    ...
    for (DLNAFile tempFile : mListDLNADeviceName) {
  ...
  private void parseContent(String path, String fileName) throws Exception {
    ...
    if (contentType == CONTENT_UNKNOW) {
      NetLog.v(TAG, "throw new Exception()");
      // throw new Exception();
    ...
    if (walkType == CD_CHILD) {
      ...
      try {
        if (!mTraceFile.empty()) {
          ...
          if(dlnaFile.getName() !=null) {
            constructCurPath(dlnaFile.getName(), false);
          }else {
            constructCurPath("", false);
          }
...
//    Util.LogResRelease("DLNAMANAGER stopDlna");
  ...
  /*public static int stoDMP() {
    return DigitalMediaPlayer.getInstance().start();
  }*/
  ...
  public void notifyDeviceFound(FoundDeviceEvent event) {
    ...
    if (MediaMainActivity.mIsDlnaAutoTest) {
      if (name.toLowerCase(Locale.ROOT).startsWith("goldendms")) {
  ...
  public void notifyDeviceLeft(LeftDeviceEvent event) {
    ...
    for (DLNAFile tempDevice : mListDLNADeviceName) {
...
//    }
  ...
  public List<String> getSubTitleList(String nameSuffix) {
    ...
    try {
      for (String name : mSubTitleContentTable.keySet()) {
        String tmpName = name.toLowerCase(Locale.ROOT);
...
//    }
  ...
  public void createDlnaFile(String name) {
    if (name == null){
        NetLog.d(TAG, "createDlnaFile name == null");
        return;
    }
    try {
      File f = new File(FileSystemPath.LINUX_TMP_PATH+"/mtk/dlna_success.txt");
      String nameTemp = name.toLowerCase(Locale.ROOT);
      if (!f.exists() && nameTemp.startsWith("goldendms")) {
        f.createNewFile();
      }
    } catch (Exception e) {
      if (localLOGV) {
  ...
  public void deleteDlnaFile(String name) {
    if (name == null){
        NetLog.d(TAG, "deleteDlnaFile name == null");
        return;
    }
    try {
      File f = new File(FileSystemPath.LINUX_TMP_PATH+"/mtk/dlna_success.txt");
