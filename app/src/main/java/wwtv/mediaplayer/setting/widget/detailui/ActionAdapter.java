...
import com.mediatek.wwtv.mediaplayer.mmp.commonview.ListAdapterMenu;
...
public class ActionAdapter extends BaseAdapter implements ScrollAdapter,
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            mViewHolder.title = (TextView) convertView.findViewById(R.id.action_title);
            mViewHolder.description = (TextView) convertView.findViewById(R.id.action_description);
            mViewHolder.checkmarkView = (ImageView) convertView.findViewById(R.id.action_checkmark);
            mViewHolder.indicatorView = (ImageView) convertView.findViewById(R.id.action_icon);
            mViewHolder.chevronView = (ImageView) convertView.findViewById(R.id.action_next_chevron);
            mViewHolder.chevronBackgroundView = convertView.findViewById(R.id.action_next_chevron_background);
        ...
        mViewHolder.description.setText(action.getDescription());
        mViewHolder.description.setVisibility(
                TextUtils.isEmpty(action.getDescription()) ? View.GONE : View.VISIBLE);
        mViewHolder.title.setText(action.getTitle());
        mViewHolder.checkmarkView.setVisibility(action.isChecked() ? View.VISIBLE : View.INVISIBLE);

        setIndicator(mViewHolder.indicatorView, action);

        mViewHolder.chevronView.setVisibility(action.hasNext() ? View.VISIBLE : View.GONE);

        mViewHolder.chevronBackgroundView.setVisibility(action.hasNext() ? View.VISIBLE : View.INVISIBLE);

        final Resources res = convertView.getContext().getResources();
        if (action.hasMultilineDescription()) {
            mViewHolder.title.setMaxLines(res.getInteger(R.integer.action_title_max_lines));
            mViewHolder.description.setMaxHeight(
                    getDescriptionMaxHeight(convertView.getContext(), mViewHolder.title, mViewHolder.description));
        } else {
            mViewHolder.title.setMaxLines(res.getInteger(R.integer.action_title_min_lines));
            mViewHolder.description.setMaxLines(
...
/*
    ...
    */
    ...
    private static synchronized Integer getDescriptionMaxHeight(Context context, TextView title,
            TextView description) {
        Log.d(TAG, "getDescriptionMaxHeight,description=="+description);
