  ...
  protected DlnaFilesManager(Context context) {
    ...
    mOperator.setOnFileEventListener(new FileEventListener() {
      ...
      public void onFileLeft(FileEvent event) {
        ...
        List<DLNAFile> files = event.getFileList(getType());
...
//          }
  ...
  private void addFiles(List<DLNAFile> files, int flag) {
    if (files == null) {
      return;
    }
    if (MediaMainActivity.mIsDlnaAutoTest) {
  ...
  public static synchronized DlnaFilesManager getInstance(Context context) {
    if (sInstance == null) {
