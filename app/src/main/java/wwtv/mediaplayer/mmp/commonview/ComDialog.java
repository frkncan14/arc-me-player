...
//    }
  ...
  public void setItemEnableState(String content, boolean enabled) {
    if (mDataList != null) {
      for (MenuFatherObject dataList:mDataList) {
        if (dataList.equals(content)) {
          dataList.enable = enabled;
          break;
        }
      }
    }
  ...
  private void setListener() {
    mAdapterMenu.setMyOffsetListener(new MyOffsetListener() {
      ...
      public void offset(int offset) {
        ...
        MtkLog.d(TAG,"setListener ,selectPosition =="+selectPosition);
      }
    });
  ...
  private void setDialogParams() {
    ...
    int menuHight = (menuWidth * 5 / 3 + (int) (ScreenConstant.SCREEN_HEIGHT * 0.010));
  ...
  public void setWindowPosition() {
    Window window = getWindow();
    WindowManager.LayoutParams lp = window.getAttributes();
    lp.x = ScreenConstant.SCREEN_WIDTH / 2 - menuWidth / 2 - marginX + 1;
    lp.y = marginY;
    window.setAttributes(lp);
  }
  ...
  public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2,
    ...
    MtkLog.d(TAG,"onItemSelected ,selectPosition =="+selectPosition);
    ...
    if (selectView != null && !selectView.equals(arg1)) {
...
//          .setTextColor(Color.WHITE);
  ...
  public void onNothingSelected(AdapterView<?> arg0) {
  	MtkLog.i(TAG,"onNothingSelected()");
  }
  ...
  public boolean onKeyDown(int keyCode, KeyEvent event) {
    ...
    MtkLog.d(TAG,"position=="+position+",offset=="+offset);
...
//            MtkLog.i(TAG,"position:" + position + "--offset:" + offset
//                + "--hasNext:" + mDataList.get(position).hasnext);
//            selectView.findViewById(R.id.mmp_menulist_img).setVisibility(
//                View.VISIBLE);
//          }
//          ((TextView) selectView.findViewById(R.id.mmp_menulist_tv))
...
//        }
      ...
      case KeyMap.KEYCODE_MTKIR_RED: //down
        if (!Util.isTTSEnabled(mContext)){
            break;
        }
        boolean hasEnableItem = false;
        for (int i = position + 1; i < mAdapterMenu.getCount(); i++){
            if (mDataList.get(i).enable){
                hasEnableItem = true;
                break;
            }
        }
        if ((position == mAdapterMenu.getCount() - 1) || !hasEnableItem) {
          int lastenabled = getLastEnableIndex();
          MtkLog.d(TAG,"lastenabled=="+lastenabled);
      ...
      case KeyMap.KEYCODE_MTKIR_GREEN: //up
        if (!Util.isTTSEnabled(mContext)){
            break;
        }
        if (selectPosition == PAGE_SIZE){
            ...
            break;
        } else if (selectPosition == 0 && getCount() > PAGE_SIZE) {
