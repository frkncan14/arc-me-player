...
public class PhotoPlayActivity extends MediaPlayActivity {
  ...
  private static final int MESSAGE_PHOTO_DRAW = 8;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
      switch (keyCode) {
        ...
        case KeyMap.KEYCODE_MTKIR_GUIDE:
          if ( msContext instanceof PhotoPlayActivity) {
  ...
  private void showSleepMenuDialog() {
    ...
    menuDialogSleepTime = new MenuListView(this, list, mListener, null);
...
//        mSleepdialog.updateValue(true);
  ...
  private void loadImageDone(PhotoUtil bitmap) {
    ...
    if (null != vShowView) {
      if (isRealGif){
        mHandler.sendEmptyMessageDelayed(MESSAGE_PHOTO_DRAW, 100);
      } else {
        vShowView.run();
      }
    }
