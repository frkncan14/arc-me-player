...
//	private InputSourceManager mSourceManager;
	...
	private Map<String, Integer> dumyData = new HashMap<String, Integer>();
...
//	}
	...
	public boolean isConfigVisible(String cfgid){
		boolean flag = mTvConfig.isConfigVisible(cfgid) == MtkTvConfigType.CFGR_VISIBLE ? true
				: false;
        MtkLog.d("TVContent", "isConfigVisible,flag == " + flag);
		return flag;
	}
...
*/
	...
	public boolean isAusCountry() {
		String country = mTvConfig.getCountry();
		return isEURegion()&& country.equalsIgnoreCase(MtkTvConfigType.S3166_CFG_COUNT_AUS);
	}

	public boolean isNorCountry() {
		String country = mTvConfig.getCountry();
		return isEURegion()&& country.equalsIgnoreCase(MtkTvConfigType.S3166_CFG_COUNT_NOR);
	}

	// is france
	public boolean isFraCountry() {
		String country = mTvConfig.getCountry();
		return isEURegion()&& country.equalsIgnoreCase(MtkTvConfigType.S3166_CFG_COUNT_FRA);
	}
...
//	}
	...
	public static void freeBackupDVBSOP() {
		MtkLog.d(TAG,"freeBackupDVBSOP(),dvbsLastOP:" + dvbsLastOP);
		dvbsLastOP = -1;
	}

	public static void backUpDVBSsatellites() {
		MtkLog.d(TAG,"backUpDVBSsatellites()=");
		...
		MtkLog.d(TAG,"backUpDVBSsatellites()=" + mDvbsSatSnapShotId);
	}

	public static void restoreDVBSsatellites() {
		MtkLog.d(TAG,"restoreDVBSsatellites()=" + mDvbsSatSnapShotId);
		if (mDvbsSatSnapShotId != -1) {
			int result = MtkTvDvbsConfigBase.restoreSatlSnapshot(mDvbsSatSnapShotId);
			MtkLog.d(TAG,"restoreDVBSsatellites(),result:" + result);
		}
	}

	public static void freeBachUpDVBSsatellites() {
		MtkLog.d(TAG,"freeBachUpDVBSsatellites()=" + mDvbsSatSnapShotId);
		if (mDvbsSatSnapShotId != -1) {
