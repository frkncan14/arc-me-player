...
public final class ScrollAdapterView extends AdapterView<Adapter> {
    ...
    public interface OnScrollListener {
         ...
         */
        void onScrolled(View view, int position, float mainPosition, float secondPosition);
    }
    ...
    private static class RecycledViews {
        ...
        View getView(int type) {
            if (mViews != null && type >= 0 && type < mViews.length) {
                ...
                return !array.isEmpty() ? array.remove(array.size() - 1) : null;
            }
            return null;
        }
    }
    ...
    private final List<ExpandedView> mExpandedViews = new ArrayList<ExpandedView>(4);
            ...
            if (hasScrollPosition(centerMain, getSize(view), scrollCenter)
                ...
                if (findNext) {
                    ...
                    if (mItemsOnOffAxis == 1) {
                        Log.d(TAG,"don't look in second axis if it's not grid");
    ...
    private boolean childHasFocus(int expandableViewIndex) {
        ...
        return v != null && v.expandedView.hasFocus();
    }
    ...
    private void fillVisibleViewsInLayout() {
        while (fillOneRightChildView(true)) {
            Log.d(TAG,"fillOneRightChildView true");
        }
        while (fillOneLeftChildView(true)) {
            Log.d(TAG,"fillOneLeftChildView true");
        }
    ...
    public boolean arrowScroll(int direction, int repeats) {
        if (DBG) {
            Log.d(TAG, "arrowScroll " + direction);
        }
    ...
    protected boolean handleArrowKey(int direction, int repeats, boolean forceFindNextExpandable,
        ...
        if (currentTop.equals(currentExpandable) && focused != null && !forceFindNextExpandable) {
            // find next focused inside expandable item
            View v = focused.focusSearch(direction);
            if (v != null && !v.equals(focused) && isDescendant(currentTop, v)) {
        ...
        if (!currentTop.equals(currentExpandable) && !forceFindNextExpandable) {
            ...
            if (nextTop != null && nextTop.equals(currentTop)) {
    ...
    private void transferFocusTo(View topItem, int direction) {
        ...
        if (topItem.equals(oldSelection)) {
            return;
        }
    ...
    private static ExpandedView findExpandedView(List<ExpandedView> expandedView, int index) {
    ...
    private ExpandedView getOrCreateExpandedView(int index) {
        ...
        View v = mExpandAdapter.getView(index, recycleView, this);
        if (v == null) {
            return null;
        }
        ...
        measureChild(v);
        if (DBG) {
            Log.d(TAG, "created new expanded View for " + index + " " + v);
        }
    ...
    private void updateViewsLocations(boolean onLayout) {
        ...
        for (int j = 0; j < expandedCount;) {
            ...
            ExpandedView v = mExpandedViews.get(j);
            if (!v.equals(thisExpanded) && !v.equals(nextExpanded) && !v.equals(previousExpanded)) {
        ...
        for (int j = 0, size = mExpandedViews.size(); j < size; j++) {
            ExpandedView v = mExpandedViews.get(j);
            int start = v.equals(thisExpanded) ? expandedStart : nextExpandedStart;
            if (!(v.equals(previousExpanded) || v.equals(nextExpanded) && progress == 1f)) {
    ...
    public List<OnItemChangeListener> getOnItemChangeListeners() {
