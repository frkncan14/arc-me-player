...
public class SambaManager {
  ...
  private static String TAG = "SambaManager";
  ...
  private final List<SmbFile> mListSmbFile = new LinkedList<SmbFile>();
  private final Map<Integer, SmbFileFilter> mFilter = new Hashtable<Integer, SmbFileFilter>();
  private static Map<String, LoginInfo> mNamePWDTable = new Hashtable<String, LoginInfo>();
  ...
  private static boolean isNeedFile(String name, String[] suffixs) {
    boolean need = false;
    try {
      String tmpFileName = name.toLowerCase(Locale.ROOT);
  ...
  */
   ...
   */
  public static Map<String, LoginInfo> getHashTable() {
...
//    jcifs.Config.setProperty("jcifs.netbios.baddr", "172.26.174.255");
  ...
  private boolean isValidFilterType(int type) {
    return !(type != TYPE_ALL && type != TYPE_VIDEO && type != TYPE_AUDIO
   ...
   */
  public List<String> getSmbFileList(String path)
      throws MalformedURLException, SmbException, SmbAuthException,
    ...
    for (SmbFile smbFileTmp : fList) {
      /* filter the format 'D$/' */
      if (smbFileTmp.getName().endsWith("$/")){
        continue;
      }
      mListSmbFile.add(smbFileTmp);
      String filePath = smbFileTmp.getPath();
   ...
   * @throws UnknownHostException
   */
  public List<String> getSmbFileList(String path, int filterType)
      throws MalformedURLException, SmbException, SmbAuthException,
      UnknownHostException {
    LinkedList<String> listSmbFilePath = new LinkedList<String>();
    if (path == null) {
    ...
    for (SmbFile smbFileTmp : fList) {
      NetLog.d(TAG, "getSmbFileList2 smbFile:" + smbFile.getName());
      /* filter the format 'D$/' */
      if (smbFileTmp.getName().endsWith("$/")){
        continue;
      }
...
//    NetLog.v(taglogin, "[getSmbFileIns] call: " + Log.getStackTraceString(new Throwable()));
   ...
   *
   */
  public List<String> sortByName(List<String> linkedList) {
