...
public class WifiWpsConnect {
	 ...
	 *
	 */
	public static synchronized WifiWpsConnect getInstance(Context context) {
		if(mWpsConnect == null) {
	...
	public List<WifiAccessPoint> getWpsScanResultList() {
		...
		if (localLOGV) {
            NetLog.d(TAG, "[WifiWpsConnect][getWpsScanResultList]: Scan Result List size: " + total);
        }
		for(i = 0; i < total; i++) {
			if(!WifiUtil.isSupportWps(mAccessPoints.get(i).getAccessResult())) {
				if (localLOGV){
                    NetLog.d(TAG, "[WifiWpsConnect][getWpsScanResultList]: " +  mWifiscanResultList.get(i).SSID +
...
//		return (mWifiManager.getPinCode() == null) ? null : mWifiManager.getPinCode();
	...
	public boolean enableWpsPinLink(String bssid, String pinCode) {
		...
		if(pinCode == null || bssid == null) {
			if (localLOGV) {
                NetLog.d(TAG, "[WifiWpsConnect][enableWpsPinLink]: Argument error.");
            }
			return false;
		}
